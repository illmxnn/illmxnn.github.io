...
## 30. Tabs-growth: stale-кэш, а не layout (2026-09-26)

> Вердикт юзера со скрином: таб «Детали» на десктопе всё ещё увеличивает весь див, хотя grid-stack (§28) уже в проде. Код выглядел верным — воспроизвели вживую и нашли виновника замерами.

- **Root cause — stale-кэш `style.css`, не layout**: свежий замер 1280 (networkidle + `document.fonts.ready` + догрузка картинок) показал grid-stack применён (`.tabs display:grid`, скрытая панель — `display:block + visibility:hidden`) и 9/9 высота кейса, высота `.tabs` и scrollY const при переключении DOM-кликом без автоскролла. Симуляция докомпонового CSS (`.tabs{display:block}` + `[hidden]{display:none}` — ровно то, что отдаёт закэшированный файл) воспроизвела рост 9/9: +60…+170px, «Детали» везде taller (там `ul.case__points`), Обзор везде 556.8. Догрузка картинок/шрифты отпали: фреймы фиксированы 380/340, обе панели масштабируются одними шрифтами, блок всё равно = max. Подозрение на кэш подтверждает и разметка: `<link href="style.css">` был без версии — браузер имел право отдать файл до grid-stack.
- **Фикс — cache-busting**: `style.css?v=20260926` в `index.html` (единственный локальный layout-ассет; JS табов инлайн, three.js — внешний CDN). Layout не тронут — он уже верный.
- **Доказательство после фикса**: 9/9 высота кейса + scrollY + высота документа const (726.8 / 678 / 703.5 / 616.8 / 703.5 / 644.5 / 678 / 646.5 / 678); скрины кейса 1 Обзор vs Детали 1280 — высота 726.796875 = 726.796875; 390 снят; HScroll 0 @1280/@390; JS-ошибок 0 (в песочнице 6× `ERR_CONNECTION_RESET` только на woff2 `fonts.gstatic` — egress среды, не сайта).
- **Бюджеты**: HScroll 0 @1280/@390, JS-ошибок 0, `diff --check` clean, кадры не тронуты, без коммита/пуша.

---

## 31. Case-balance: HUD-пилюля прижата к низу колонки (2026-09-26)

> Вердикт юзера со скрином: левая колонка кейса (слайдер) короче правой текстовой — под HUD-пилюлей огромная пустота.

- **Причина**: `.case` — grid 2 колонки, правая (табы grid-stack + результат) выше левой; `.shots` лежал блоком по контенту, а `.shots__bar` — с фиксированным `margin-top: 16px` сразу под подписью. Свободное место строки оставалось под пилюлей.
- **Фикс (только CSS, десктоп)**: `.case { align-items: stretch }` явно + `.case__media { align-self: stretch }`; на `min-width: 1001px` (пока 2 колонки) `.shots` — flex-столбец на всю высоту (`flex: 1 1 auto; height: 100%`), подпись держит минимум `margin-bottom: 16` по шкале, а `.shots__bar` — `margin-top: auto`: слайдер сверху, подпись + пилюля внизу. Низ пилюли и низ правой колонки — на одном уровне. Высоты слайдов 380/340, peek 0, grid-stack табов, стрелки/клавиатура/лайтбокс — не тронуты.
- **Mobile (1 колонка, ≤1000px)**: без изменений — `.shots` по контенту, пилюля с прежним `margin-top: 16`, порядок слайдер → подпись → пилюля тот же, новых отступов нет.
- **Cache-busting**: `style.css?v=20260926 → v=20260926-2` в `index.html` (урок §30: новый CSS — новая версия, иначе вернётся stale-кэш).
- **Бюджеты**: низ пилюли vs низ правой колонки ±8px (замер 1280, 2–3 кейса), скрины 1280 + 390, HScroll 0 оба, консоль 0, `diff --check` clean, кадры не тронуты, без коммита/пуша.

---

## 32. HUD-group: подпись+пилюля одним блоком у низа (2026-09-26)

> Вердикт юзера со скрином: `margin-top: auto` уехал только на пилюле — подпись осталась под слайдером, пилюля сиротой внизу. Выглядит сломанно.

- **Причина**: в §31 `auto` висел на `.shots__bar`, а `.shots__cap` держалась сразу под вьюпортом (`margin: 12px 2px 16px`). Свободное место строки уходило между подписью и пилюлей — группа рвалась.
- **Фикс (только CSS, десктоп)**: `margin-top: auto` перенесён с пилюли на подпись — первый элемент нижней группы. `.case__media .shots__cap` — `margin-top: auto; margin-bottom: 0; padding-top: 12px` (`padding` держит минимум 12 по шкале, когда свободного места нет и `auto` схлопывается в 0); `.case__media .shots__bar` — `margin-top: 12px`: подпись + пилюля едут вниз вместе, пустота — между слайдером и группой, низ пилюли и низ правой колонки — на одном уровне. Зазор подпись→пилюля 12px (шкала §5). Высоты 380/340, peek 0, grid-stack, лайтбокс, свайп — не тронуты.
- **Mobile (1 колонка, ≤1000px)**: без изменений — базовые `.shots__cap { margin: 12px 2px 0 }` и `.shots__bar { margin-top: 16px }`, порядок слайдер → подпись → пилюля тот же.
- **Cache-busting**: `style.css?v=20260926-2 → v=20260926-3` в `index.html` (урок §30: новый CSS — новая версия, иначе вернётся stale-кэш).
- **Бюджеты**: подпись+пилюля смежны 8–16px, низ пилюли vs низ правой колонки ±8px (замер 1280, 9/9), скрины 1280 + 390, HScroll 0 оба, консоль 0, `diff --check` clean, кадры не тронуты, без коммита/пуша.
